...
for i in range(int(size)):
        ...
        f.write("" +ip[i]+ "    " +cont[i]+ ""+'\n')
        f.close()
print commands.getoutput('cat a.txt') 
print commands.getoutput('sudo docker cp -a /var/www/cgi-bin/a.txt '+cont[0]+':/')
print commands.getoutput('sudo docker exec -u 0 '+cont[0]+' /bin/sh -c  "cat  /a.txt >>/etc/hosts"')
